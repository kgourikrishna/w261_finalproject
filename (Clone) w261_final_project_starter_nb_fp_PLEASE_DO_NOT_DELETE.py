...
# MAGIC %md
# MAGIC # Missing Value Analysis and Dataset Information
...
unique_counts_df = df_otpw.select([approx_count_distinct(c).alias(c) for c in df_otpw.columns]).toPandas()
null_counts_df = df_otpw.select([count(when(isnan(c) | col(c).isNull(), c)).alias(c) for c in df_otpw.columns]).toPandas()

for col, dtype in df_otpw.dtypes:
    print('column:', col)
    print('type:', dtype)
    print('num_unique:', unique_counts_df[col].values[0])
    print('num_null:', null_counts_df[col].values[0])
    print()
...
DoW = [day_of_week_map[str(d[0])] for d in DoW]
plt.bar(DoW, vals)
...
df_otpw_60m_with_ranks = df_otpw_60m_2018.join(other=pageranks_2018,
                                               ...
                                               how='left')
df_otpw_60m_with_ranks

# COMMAND ----------

pagerank_delay_df = df_otpw_60m_with_ranks.select(['pagerank', 'DEP_DEL15'])
